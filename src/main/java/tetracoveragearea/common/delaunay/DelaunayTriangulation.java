...
public class DelaunayTriangulation {
	...
	private void updateNeighbor(Triangle addedTriangle, Triangle deletedTriangle, Point pointToDelete) {
		...
		if (pointToDelete.equals(delA)) {
			...
			// AC-BC || CA-BC
				...
				addedTriangle.setAcTriangle(deletedTriangle.getBcTriangle());
			}
			// BC-BC || CB-BC
			else {
			...
			// AC-AC || CA-AC
				...
				addedTriangle.setAcTriangle(deletedTriangle.getCaTriangle());
			}
			// BC-AC || CB-AC
			else {
		...
		// equals c
			...
			// AC-AB || CA-AB
				...
				addedTriangle.setAcTriangle(deletedTriangle.getAbTriangle());
			}
			// BC-AB || CB-AB
			else {
	...
	private void updateNeighbor(Triangle addedTriangle1, Triangle addedTriangle2) {
		...
		if (A1.equals(A2)) {
			...
			// A1B1-A2C2
				...
				addedTriangle1.setAbTriangle(addedTriangle2);
				addedTriangle2.setAcTriangle(addedTriangle1);
			}
			// A1C1-A2B2
			else if (C1.equals(B2)) {
				addedTriangle1.setAcTriangle(addedTriangle2);
				addedTriangle2.setAbTriangle(addedTriangle1);
			}
			// A1C1-A2C2
			else {
	...
	private void insertCollinear(Point p, int res) {
		...
		case Point.INFRONTOFA:
			...
			t.setBcTriangle(tp);
			tp.setAcTriangle(t);
			t.setAcTriangle(firstT);
			firstT.setBcTriangle(t);
			tp.setBcTriangle(firstT.getAbTriangle());
			firstT.getAbTriangle().setAcTriangle(tp);
			firstT = t;
			firstP = p;
			break;
		case Point.BEHINDB:
			...
			lastT.setAcTriangle(t);
			t.setAcTriangle(tp);
			tp.setBcTriangle(t);
			tp.setAcTriangle(lastT.getAbTriangle());
		...
		case Point.ONSEGMENT:
			...
			u.getBcTriangle().setAcTriangle(t);
			t.setAcTriangle(u);
			u.setBcTriangle(t);
			tp.setAcTriangle(u.getAbTriangle().getCaTriangle());
			u.getAbTriangle().getCaTriangle().setBcTriangle(tp);
			tp.setBcTriangle(u.getAbTriangle());
			u.getAbTriangle().setAcTriangle(tp);
			if (firstT == u) {
				firstT = t;
			}
			break;
	...
	/**
	 * Проверяет, не лежит ли точка на линии. Если лежит, то...
	 * @param t
	 * @param p
	 * @return
	 */
	...
	private Triangle extendOutside(Triangle t, Point p) {
		...
		if (p.pointLineTest(t.getA(), t.getB()) == Point.ONSEGMENT) {
			...
			hp.getBcTriangle().setAcTriangle(hp);
			hp.setAcTriangle(t);
			t.setBcTriangle(hp);
			return dg;
		}
	...
	/**
	 * Обновляет границу внешней рамки области точек (если появилась точка, выходящая за границы этой области)
	 * @param p
	 */
	...
	public List<Triangle> getTriangulation() {
		...
		triangulation.removeIf(triangle -> triangle.isHalfplane());
